...
class AndroidSubscriptionDialog extends StatefulWidget {
  final Function(String) onSubscribe;
  final List<ProductDetails> products;

  const AndroidSubscriptionDialog({
    Key? key,
  ...
  @override
  _AndroidSubscriptionDialogState createState() => _AndroidSubscriptionDialogState();
}

class _AndroidSubscriptionDialogState extends State<AndroidSubscriptionDialog> {
  int _currentImageIndex = 0;
  String _selectedSubscriptionType = StoreProducts.monthlySubAndroid;
  ...
  final List<String> _benefitDescriptions = [
    'Generate unlimited thesis statements',
    'Access various writing styles and formats',
    ...
    return SizedBox(
      ...
      child: FlutterCarousel(
        options: CarouselOptions(
          ...
          showIndicator: false, // Hide the indicators
        ),
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildTermItem('Payment will be charged to your Google Play account at confirmation of purchase'),
              ...
              _buildTermItem('You can manage and cancel your subscriptions by going to your account settings on the Google Play Store'),
              ...
              Row(
                ...
                children: [
                  ...
                  _buildLinkButton('Terms of Use', 'https://sites.google.com/view/thesis-generator'),
                ],
              ),
            ],
          ),
  ...
  Widget _buildSubscriptionOptions() {
    ...
    final monthlyProduct = widget.products.firstWhere(
          (product) => product.id == StoreProducts.monthlySubAndroid,
    );

    final yearlyProduct = widget.products.firstWhere(
          (product) => product.id == StoreProducts.yearlySubAndroid,
    );
        ...
        children: [
          Expanded(
            child: _buildSubscriptionOption(
              monthlyProduct,
              'Monthly',
              isSelected: _selectedSubscriptionType == StoreProducts.monthlySubAndroid,
            ),
          ),
          Expanded(
            child: _buildSubscriptionOption(
              yearlyProduct,
              'Yearly',
              isSelected: _selectedSubscriptionType == StoreProducts.yearlySubAndroid,
              isBestValue: true,
            ),
          ),
        ],
  ...
  Widget _buildSubscriptionOption(
    ...
    InkWell(
    onTap: () => setState(() {
    _selectedSubscriptionType = product.id;
    }),
    child: Padding(
    padding: EdgeInsets.all(16),
    child: Column(
    ...
    return ElevatedButton(
      onPressed: () async {
        try {
          ...
          await widget.onSubscribe(_selectedSubscriptionType);
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  String _getPeriod(String productId) {
    if (productId == StoreProducts.yearlySubAndroid) return 'yearly';
    if (productId == StoreProducts.monthlySubAndroid) return 'monthly';
    return 'monthly';
  }

  String _getSavingsText(ProductDetails product) {
    if (product.id == StoreProducts.monthlySubAndroid) {
      return 'Save 40%';
    }
    if (product.id == StoreProducts.yearlySubAndroid) {
